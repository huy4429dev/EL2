...
const RoomPage = () => {
    ...
    return (
        <div>
            <Breadcrumb items={["Phòng học"]}>
                <Space>
                    <Button
                        onClick={() => navigate("/admin/branch/room/new")}
                        className="ml-auto"
                        type="primary"
                    >
                        Thêm phòng học
            ...
            >
                <Table
                    footer={() => `Tổng số phòng học ${pagination.total}`}
                    ...
                    columns={[
                        ...
                        },
                        {
                            ...
                            render: (text, record, index) => {
                                const indexBranch = branches.findIndex(i => i.id === record.branch_id)
                                return (
                                    <span>
                                        {branches[indexBranch]?.name}
                                    </span>
                                );
                            },
                        },
                            ...
                            render: (text, record, index) => {
                                return (
                                    <div key={index}>
                                        <Link
                                            className="mr-2"
                                            to={`/admin/branch/room/${record.id}`}
                                        >
                                            <EditFilled />
                                        </Link>
                                        <Popconfirm
