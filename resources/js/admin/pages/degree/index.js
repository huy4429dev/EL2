...
const DegreePage = () => {
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete("/api/admin/degree/" + id);
            setPagination({
                ...pagination,
                total: pagination.total - 1
            })

            message.success("Xoá học vị thành công");
    ...
    return (
        <div>
            <Breadcrumb items={["Học vị"]}>
                <Space>
                    <Button
                        onClick={() => navigate("/admin/degree/new")}
                        className="ml-auto"
                        type="primary"
                    >
                        Thêm học vị
            ...
            >
                <Table
                    footer={() => `Tổng số học vị ${pagination.total}`}
